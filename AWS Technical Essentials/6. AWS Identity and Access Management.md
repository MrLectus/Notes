...
After looking at the JSON, you can see that this policy allows the IAM user to change their own IAM password (*iam:ChangePassword*) and get
information about their own user (*iam:GetUser*). It only permits the user to access their own credentials because the resource restricts
access with the variable substitution *${aws:username}*.
